...
# this query can be used to "fix" characters affected by this error


UPDATE azth_1_chars.characters AS a RIGHT JOIN (
SELECT guid,cAccount FROM azth_auth.account_transfer WHERE guid IN (SELECT guid FROM azth_1_chars.characters WHERE account = 523) AND cStatus = 1
) AS b
